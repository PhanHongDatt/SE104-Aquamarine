  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message); // Hiển thị lỗi từ Prisma (ví dụ: thiếu bảng, sai URL)
      }
    } catch (e) {
    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Danh mục đơn vị đo lường trong hệ thống (Admin)</p>
        </div>
        <Button 
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-zinc-500 italic">
            Chưa có dữ liệu đơn vị tính. Hãy nhấn "Thêm đơn vị" hoặc chạy seed.
          </div>
        ) : (
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/60 font-semibold text-zinc-600">
                  <th className="px-6 py-4">Mã DVT</th>
                  <th className="px-6 py-4">Tên đơn vị tính</th>
