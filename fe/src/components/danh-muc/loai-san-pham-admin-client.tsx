            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 font-sans">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-zinc-700 ml-1">Đơn vị tính mặc định</label>
	                <select
	                  {...register("maDVT")}
	                  className="w-full max-w-full h-11 px-4 bg-white border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-semibold appearance-none shadow-sm cursor-pointer truncate overflow-hidden"
	                >
	                  <option value="">-- Chọn đơn vị --</option>
	                  {units.map(u => (
	                    <option key={u.maDVT} value={u.maDVT} className="truncate" title={u.tenDVT}>{u.tenDVT}</option>
	                  ))}
	                </select>
