        ...
        <table className="w-full text-sm text-left font-sans">
          <thead>
            <tr className="border-b border-zinc-100 bg-zinc-50/50 font-montserrat text-[11px] uppercase tracking-wider text-zinc-500">
	              ...
	              <th className="px-6 py-5 font-bold">Tên Nhà Cung Cấp</th>
	              <th className="px-6 py-5 font-bold">Địa Chỉ</th>
	              <th className="px-6 py-5 font-bold">Số Điện Thoại</th>
          ...
          <tbody className="divide-y divide-zinc-100">
            {suppliers.length === 0 ? (
              <tr>
	                <td colSpan={6} className="px-6 py-20 text-center text-zinc-400 italic">Chưa có dữ liệu nhà cung cấp.</td>
              </tr>
            ) : (
	              suppliers.map((ncc, index) => (
	                <tr key={ncc.maNCC} className="hover:bg-primary/[0.01] transition-colors group">
	                  <td className="px-6 py-4 text-center font-mono text-xs text-zinc-400">{index + 1}</td>
